...
type BadgeScheme = "red" | "blue";

export type OGContent = {
  badge: { emoji: string; text: string; scheme: BadgeScheme };
  headline: {
    line1: string;
    /** 선택: line1 뒤에 붙는 파란 강조 텍스트 */
    line1Accent?: string;
    line2: string;
  };
  /** 점 구분자로 렌더되는 서브 라인 항목들 */
  subtitle: string[];
};
...
const BADGE_COLORS: Record<BadgeScheme, { bg: string; fg: string; border: string }> = {
  red: { bg: "#fef2f2", fg: "#dc2626", border: "rgba(254, 202, 202, 0.5)" },
  blue: { bg: "#eff6ff", fg: "#2563eb", border: "rgba(191, 219, 254, 0.5)" },
};
...
export async function renderOGImage(content: OGContent): Promise<ImageResponse> {
  ...
  const b = BADGE_COLORS[content.badge.scheme];
    ...
    (
      ...
      >
        ...
        <div style={{ display: "flex", position: "relative", zIndex: 10 }}>
          ...
          >
            <span style={{ fontSize: 32 }}>{content.badge.emoji}</span>
        ...
        >
          ...
          >
            <div style={{ display: "flex", flexWrap: "wrap" }}>
              <span>{content.headline.line1}</span>
              {content.headline.line1Accent && (
                <span style={{ color: "#2563eb" }}>
                  {content.headline.line1Accent.startsWith(" ") ||
                  content.headline.line1.endsWith(" ")
                    ? content.headline.line1Accent
                    : `\u00A0${content.headline.line1Accent}`}
                </span>
              )}
            </div>
            <div>{content.headline.line2}</div>
          </div>
          <div
            style={{
              fontSize: 36,
          ...
          >
            {content.subtitle.flatMap((item, i) => {
              ...
              els.push(<span key={`t-${i}`}>{item}</span>);
              return els;
            })}
          </div>
        </div>
